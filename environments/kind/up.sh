...
trap receipt EXIT
kind create cluster --name "$cluster" --image "$KIND_IMAGE" --config "$state_dir/cluster.yaml" --kubeconfig "$kubeconfig" --wait 0s --retain
sha256 "$kubeconfig" > "$state_dir/kubeconfig.sha256"
kubectl --kubeconfig "$kubeconfig" --context "kind-$cluster" get --raw /readyz >/dev/null
if [[ "$DEFAULT_CNI_DISABLED" == false ]]; then kubectl --kubeconfig "$kubeconfig" --context "kind-$cluster" wait --for=condition=Ready node --all --timeout=180s; fi
