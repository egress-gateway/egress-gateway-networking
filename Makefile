...
.PHONY: check fmt e2e e2e-up e2e-test e2e-down
check:
	@test -z "$$(gofmt -l cmd test/e2e/environment test/e2e/suite)" || { echo 'Run make fmt'; exit 1; }
...
fmt:
	gofmt -w cmd test/e2e/environment test/e2e/suite
