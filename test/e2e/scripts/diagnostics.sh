...
for ns in networking-egress networking-gateway networking-controls networking-np networking-np-other networking-dns calico-system tigera-operator; do
...
if k get namespace networking-dns >/dev/null 2>&1; then
  k -n networking-dns get networkpolicy,serviceentry -o json | jq '[.items[]|{kind,metadata:{name:.metadata.name,namespace:.metadata.namespace},spec,status}]' > "$artifacts/dns-configuration.json" || true
fi
